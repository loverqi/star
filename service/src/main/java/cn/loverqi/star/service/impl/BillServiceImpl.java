...
public class BillServiceImpl extends BaseServiceImpl<Bill> implements BillService {
    ...
    @Override
    public ResponsePageData<Bill> selectByExampleWithRowbounds(Example example, int page, int pageSize) {
        ResponsePageData<Bill> pageDatas = super.selectByExampleWithRowbounds(example, page, pageSize);
    ...
    @Override
    public List<Bill> selectByExample(Example example) {
        List<Bill> bills = super.selectByExample(example);
        for (Bill bill : bills) {
            setCreateBean(bill);
        }
